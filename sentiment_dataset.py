...
def train_input_fn(training_dir, filename, config):
    return _input_fn(training_dir, filename, config, "train")

def validation_input_fn(training_dir, filename, config):
    return _input_fn(training_dir, filename, config, "dev")

def eval_input_fn(training_dir, filename, config):
    return _input_fn(training_dir, filename, config, "eval")
...
#     print(config)
    ...
    obj = s3.Object('aiops-assignment6', json_path)
    body = obj.get()['Body'].read().decode().splitlines()
...
#         entry = json.loads(json_acceptable_string)
        ...
        if len(entry["features"]) != config["padding_size"]:
            raise ValueError(
                "The size of the features of the entry with twitterid {} was not expected".format(
                    int(entry["id"])))
        print(entry["sentiment"] / 4)
        labels.append(entry["sentiment"] / 4)
        features.append(entry["features"])

    return features, labels

def _input_fn(directory, filename, config, mode):
    ...
    # print('main yahan hu 64')
    features, labels = _load_json_file(filename, config)
    # print('line 66')
